...
Gap.prototype.startAdvertising = function(name, serviceUuids, serviceData) {
  ...
  var scanDataNameLength = name ? name.length : 0;
  if (scanDataNameLength > 0) {
    var nameBuffer = new Buffer(name);

    scanData.writeUInt8(1 + scanDataNameLength, scanDataOffset);
    scanDataOffset++;

    advertisementData.writeUInt8(0x08, scanDataOffset);
    scanDataOffset++;

    nameBuffer.copy(scanData, scanDataOffset);
    scanDataOffset += scanDataNameLength;
  }
